...
const batteriesData = require("./batteries");
...
const resolvers = {
  Query: {
    ...
    async battery(_, { id }, { user }) {
      if (user) {
        return batteriesData.filter((battery) => battery.id == id)[0];
      }
      throw new Error("Sorry, you're not an authenticated user!");
    },

    async batteries(_, { brand }, { user }) {
      if (user) {
        return batteriesData.filter((battery) => battery.brand == brand);
      }
      throw new Error("Sorry, you're not an authenticated user!");
    },
  },
